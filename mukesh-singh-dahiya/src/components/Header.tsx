...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-navy-800 dark:bg-navy-950/90">
      <div className="container-page flex h-16 items-center justify-between gap-4">
        <Link to="/" className="flex min-w-0 items-center gap-2">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-navy-900 text-sm font-bold text-cyan-400 dark:bg-cyan-400 dark:text-navy-950">
            MD
          </span>
          ...
          {navGroups.map((group) => (
            <div key={group.label} className="relative" onMouseEnter={() => setOpenGroup(group.label)}>
              ...
              {openGroup === group.label && (
                <div className="absolute left-0 top-full w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-card-lg dark:border-navy-700 dark:bg-navy-900">
                  {group.links.map((l) => (
                    <Link key={l.to} to={l.to} className="block rounded-lg px-3 py-2 text-sm text-navy-700 hover:bg-slate-50 hover:text-brand-700 dark:text-slate-200 dark:hover:bg-navy-800 dark:hover:text-cyan-400">
                      {l.label}
                    </Link>
                  ))}
                </div>
